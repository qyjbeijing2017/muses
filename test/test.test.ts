...
import { generateCode } from '../src/subshader/pass/code/glsl/glsl-generator';
...
const ctx = muses.subShaders[0].passes[0].code?.parseToAst({
...
fs.writeFileSync(
    ...
    ctx.vertex ? generateCode(ctx.vertex): 'vertex error',
);

fs.writeFileSync(
    path.join(__dirname, 'test.frag'),
    ctx.fragment ? generateCode(ctx.fragment): 'fragment error',
);
